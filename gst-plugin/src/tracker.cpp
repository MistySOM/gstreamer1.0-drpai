...
void tracker::track(const std::list<detection> &detections)
{
    ...
    struct track_map {
        ...
        std::shared_ptr<tracked_detection> t        = nullptr;
        float                              distance = 0;
    };
        ...
        if (get_duration(now, (*track_item)->seen_last) < time_threshold) {
            for (auto &det_item: detections_ptr) {
                ...
                if ((*track_item)->c == det_item->c) {
                    ...
                    if (const auto distance = (*track_item)->smooth_bbox.mix.doa_with(det_item->bbox);
                        distance < doa_threshold) {
                        // Add to the permutation
                        permutation.push_back(track_map{
                                .det      = det_item,
                                .t        = *track_item,
                                .distance = distance,
                        });
    ...
    for (const auto *const d: detections_ptr) {
...
{
    const auto now      = std::chrono::system_clock::now();
    const auto outdated = std_find_if(historical_items,
                                      [&](const auto &t) { return get_duration(now, t->seen_last) > history_length; });
...
{
    json_object j;
    j.add("minutes", history_length / SEC_PER_MIN);
