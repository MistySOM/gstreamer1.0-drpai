 ...
 ******************************************/
static uint16_t float32_to_float16(float a) { return __truncXfYf2__<float, uint32_t, 23, uint16_t, uint16_t, 10>(a); }
 ...
 ******************************************/
static float float16_to_float32(uint16_t a) { return __extendXfYf2__<uint16_t, uint16_t, 10, float, uint32_t, 23>(a); }
...
uint8_t PreRuntime::ReadAddrmapTxt(std::string addr_file)
{
    ...
    drpai_obj_info.data_inout.data_in_addr =
            drpai_obj_info.drpai_address.data_in_addr + drpai_obj_info.data_inout.start_address;
    drpai_obj_info.data_inout.data_in_size = drpai_obj_info.drpai_address.data_in_size;
    drpai_obj_info.data_inout.data_out_addr =
            drpai_obj_info.drpai_address.data_out_addr + drpai_obj_info.data_inout.start_address;
...
{
    if (str.size() >= prefix.size() && std::equal(std::begin(prefix), std::end(prefix), std::begin(str))) {
        return true;
    }
    return false;
}
...
uint8_t PreRuntime::Load(const std::string pre_dir, uint32_t start_addr, uint8_t mode)
{
    ...
    if (dir.size() >= tmp_dir.size() && dir.find(tmp_dir, dir.size() - tmp_dir.size()) != std::string::npos) {
...
int8_t PreRuntime::UpdateParamData(const s_preproc_param_t param)
{
    ...
    if (new_pre_in_shape_w == INVALID_SHAPE)
        new_pre_in_shape_w = internal_param_val.pre_in_shape_w;
    if (new_pre_in_shape_h == INVALID_SHAPE)
    ...
    if (MODE_PRE == run_mode) {
        ...
        if (new_pre_in_format == INVALID_FORMAT)
            new_pre_in_format = internal_param_val.pre_in_format;
        if (new_pre_out_format == INVALID_FORMAT)
        ...
        if ((new_pre_in_format != internal_param_val.pre_in_format) ||
            ...
            num_updated++;
#ifdef DEBUG_LOG
            std::cout << "[INFO] Changed pre_in_format: 0x" << std::hex << new_pre_in_format << "("
                      << format_string_table.at(new_pre_in_format) << ")" << std::endl;
            std::cout << "[INFO] Changed pre_out_format: 0x" << std::hex << new_pre_out_format << "("
                      << format_string_table.at(new_pre_out_format) << ")" << std::endl;
#endif
        }
        /*--------------------------*
         * Crop parameters          *
        ...
        if (!crop_included) {
            ...
            /* If one of them is INVALID_SHAPE, use the current shape */
            if (new_crop_tl_x == INVALID_SHAPE)
                new_crop_tl_x = internal_param_val.crop_tl_x;
            if (new_crop_tl_y == INVALID_SHAPE)
                new_crop_tl_y = internal_param_val.crop_tl_y;
            if (new_crop_w == INVALID_SHAPE)
                new_crop_w = internal_param_val.crop_w;
            if (new_crop_h == INVALID_SHAPE)
            ...
            if ((new_crop_tl_x != internal_param_val.crop_tl_x) || (new_crop_tl_y != internal_param_val.crop_tl_y) ||
                (new_crop_w != internal_param_val.crop_w) || (new_crop_h != internal_param_val.crop_h)) {
                UpdateCropParam(new_crop_tl_x, new_crop_tl_y, new_crop_w, new_crop_h);
                num_updated++;
                if (!resize_included)
                    new_pre_out_shape_w = new_crop_w;
                if (!resize_included)
                    new_pre_out_shape_h = new_crop_h;
#ifdef DEBUG_LOG
        ...
        if (!resize_included) {
            ...
            /* If one of them is INVALID_SHAPE, use the current shape */
            if (new_resize_w == INVALID_SHAPE)
                new_resize_w = internal_param_val.resize_w;
            if (new_resize_h == INVALID_SHAPE)
            ...
            if (new_resize_w != internal_param_val.resize_w || new_resize_h != internal_param_val.resize_h)
    ...
    if (0 == new_pre_out_shape_w)
        new_pre_out_shape_w = pre_out_shape_w;
    if (0 == new_pre_out_shape_h)
        new_pre_out_shape_h = pre_out_shape_h;
    ...
    std::cout << pre_out_shape_c << ", " << +pre_out_type_size << ")" << std::endl;
#endif
    drpai_obj_info.data_inout.data_in_size =
            new_pre_in_shape_w * new_pre_in_shape_h * pre_in_shape_c * pre_in_type_size;
    drpai_obj_info.data_inout.data_out_size =
            new_pre_out_shape_w * new_pre_out_shape_h * pre_out_shape_c * pre_out_type_size;
...
uint8_t PreRuntime::Pre(s_preproc_param_t *param, void **out_ptr, uint32_t *out_size)
{
    ...
    std::cout << "[TIME] UpdateParamData() Processing Time: " << std::fixed << std::setprecision(2) << diff << " msec."
              << std::endl;
    ...
    timespec_get(&ts_end, TIME_UTC);
    diff = timedifference_msec(ts_start, ts_end);
    std::cout << "[TIME] UpdateWeightData() Processing Time: " << std::fixed << std::setprecision(2) << diff << " msec."
              << std::endl;
    ...
    timespec_get(&ts_end, TIME_UTC);
    diff = timedifference_msec(ts_start, ts_end);
    std::cout << "[TIME] parammodified Processing Time: " << std::fixed << std::setprecision(2) << diff << " msec."
              << std::endl;
    ...
    timespec_get(&ts_end, TIME_UTC);
    diff = timedifference_msec(ts_start, ts_end);
    std::cout << "[TIME] weightmodified Processing Time: " << std::fixed << std::setprecision(2) << diff << " msec."
              << std::endl;
    ...
    std::cout << "[TIME] PreRuntime DRP-AI processing time : " << std::fixed << std::setprecision(2) << preproc_time
              << " msec" << std::endl;
    ...
    timespec_get(&ts_end, TIME_UTC);
    diff = timedifference_msec(ts_start, ts_end);
    std::cout << "[TIME] GetResult() Processing Time : " << std::fixed << std::setprecision(2) << diff << " msec"
              << std::endl;
#endif
